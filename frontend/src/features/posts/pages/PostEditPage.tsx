    ...
    try {
      if (isEdit && postId) {
        await updatePost.mutateAsync({
          id: postId,
          content,
          newImages: croppedFiles,
          deletePictureIds: Array.from(pendingDeleteIds),
        });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 space-y-4">
      ...
      {(!isEdit || existing !== undefined) && (() => {
        const existingImages = isEdit && existing
          ? existing.picture_urls
              .map((url, i) => ({ url, id: existing.picture_ids[i] }))
        ...
        return (
          <div className="rounded-xl border border-border bg-card shadow-sm dark:shadow-none p-4 space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {/* 既存画像（編集モード） */}
              {existingImages.map(({ url, id }) => (
                <div
                  key={id}
                  className="relative overflow-hidden rounded-lg border border-border"
                  style={{ width: 72, height: 72 }}
                >
                  <img src={url} alt="既存画像" className="h-full w-full object-cover" />
                  <button
                    type="button"
                    onClick={() => handleDeleteExisting(id)}
                    aria-label="この画像を削除"
